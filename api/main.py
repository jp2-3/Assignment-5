...
# API endpoints for sandwiches

@app.post("/sandwiches/", response_model=schemas.Sandwich, tags=["Sandwiches"])
def create_sandwich(sandwich, db: Session = Depends(get_db)):
...
def update_one_sandwich(sandwich_id: int, db: Session = Depends(get_db)):
...
# API endpoints for order details

@app.post("/order_details/", response_model=schemas.OrderDetail, tags=["OrderDetails"])
def create_order_detail(order_detail, db: Session = Depends(get_db)):
...
def update_one_order_detail(order_detail_id: int, order_detail, db: Session = Depends(get_db)):
...
def update_one_resource(resource_id: int, resource, db: Session = Depends(get_db)):
...
def update_one_recipe(recipe_id: int, recipe, db: Session = Depends(get_db)):
